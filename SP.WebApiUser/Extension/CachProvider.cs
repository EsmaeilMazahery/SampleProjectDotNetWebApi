...
namespace SP.WebApiMember.Extension
{
    public static class CachProvider
    {
        ...
        public static int countOnlineUsers(this IDistributedCache cache)
        {
            ...
            if (users != null)
                return users.FromByteArray<List<(int, string, DateTime)>>().Count();
            else
                return 0;
        }

        public static List<(int,string, DateTime)> getOnlineUsers(this IDistributedCache cache)
        {
            var users = cache.Get(OnlineUserKey);
            if (users != null)
                return users.FromByteArray<List<(int, string, DateTime)>>();
            else
                return new List<(int, string, DateTime)>();
        }

        public static void setOnlineUsers(this IDistributedCache cache, List<(int, string, DateTime)> users)
        {
        ...
        /// <param name="cache"></param>
        /// <param name="member"></param>
        public static bool addOnlineUsers(this IDistributedCache cache, (int, string, DateTime) user)
        {
            var usersByte = cache.Get(OnlineUserKey);
            if (usersByte != null)
            {
                var users = usersByte.FromByteArray<List<(int, string, DateTime)>>();
            ...
            {
                var users = new List<(int, string, DateTime)>();
                users.Add(user);
                cache.Set(OnlineUserKey, users.ToByteArray());
                return true;
            }
