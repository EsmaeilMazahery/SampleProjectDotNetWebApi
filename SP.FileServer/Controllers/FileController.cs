    ...
    public class FileController : Controller
    {
        ...
        public IActionResult Post_File_Upload(string PreAddress = null)
        {
            try
            {
                ...
                string[] imageExtensions = new string[] { ".rar", ".zip", ".xls", ".xlsx", ".pdf", ".doc", ".docx", ".jpg", ".jpeg", ".bmp", ".png", ".gif", ".txt", ".html", ".htm", ".css", ".7zip", ".mp3", ".ogg", ".wav", ".wma", ".7z", ".pps", ".ppt", ".pptx", ".xlr", ".ods", ".odp", ".3gp", ".avi", ".flv", ".h264", ".mkv", ".mov", ".mp4", ".mpg", ".mpeg", ".swf", ".wmv", ".odt", ".wks", ".wps" };
                ...
                // generate filename
                string root =_hostingEnvironment.ContentRootPath + "\\Files\\";
                string path = root;
                string pathThumb = "";
                string pathMedium = "";
                ...
                if (!string.IsNullOrWhiteSpace(PreAddress))
                {
                    ...
                    {
                        pathThumb = Path.Combine(path + "Thumb");

                        if (!Directory.Exists(pathThumb))
                            Directory.CreateDirectory(pathThumb);

                        pathMedium = Path.Combine(path + "Medium");
                ...
                using (var stream = new FileStream(path, FileMode.Create))
                {
                    ...
                    if (!string.IsNullOrEmpty(pathThumb))
                    {
                        Image newImageThumb = GetReducedImage(120, 120, file.OpenReadStream());
                        newImageThumb.Save(pathThumb);

                        Image newImageMedium = GetReducedImage(600, 600, file.OpenReadStream());
            ...
            catch(Exception ex)
            {
                ...
                return Ok(new Post_File_Upload_Response { result = false, message = ex.ToString() });
            }
        }
